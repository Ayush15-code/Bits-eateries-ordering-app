...
export default function MerchantDash() {
  ...
  const handlePaymentStatus = async (id, newStatus) => {
    try {
      ...
      if (newStatus === "CONFIRMED") updateData.confirmedAt = new Date().toISOString();
          ...
          <div className="space-y-4">
            {orders.filter(o => !['COLLECTED', 'REJECTED'].includes(o.status)).length === 0 ? (
              ...
              orders.filter(o => !['COLLECTED', 'REJECTED'].includes(o.status)).map(o => {
                ...
                return (
                  <div key={o.id} className={`bg-white dark:bg-gray-900 p-5 rounded-2xl shadow-md border-l-8 transition-all ${isRunning ? 'border-green-500' : 'border-orange-500'}`}>
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className={`text-[10px] px-2 py-1 rounded font-bold uppercase ${isRunning ? 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400' : 'bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400'}`}>
                        {isRunning ? "Ready to Cook" : o.status}
                      </span>
                    </div>
                    <div className="mb-4 text-xs text-gray-600 dark:text-gray-400 font-medium">
                    ...
                    <div className="flex gap-2">
                      {!isRunning ? (
                        <>
                          <button onClick={() => handlePaymentStatus(o.id, "CONFIRMED")} className="flex-1 bg-orange-600 text-white py-3 rounded-xl font-bold shadow-lg shadow-orange-600/20 active:scale-95 transition-all">Accept</button>
                        ...
                        <button onClick={() => handlePaymentStatus(o.id, "COLLECTED")} className="flex-1 bg-green-600 text-white py-3 rounded-xl font-bold shadow-lg shadow-green-600/20 active:scale-95 transition-all">✅ Mark Collected</button>
                      )}
                    </div>
                  </div>
                );
        ...
        {activeTab === 'history' && (
          <div className="space-y-4">
            <div className="flex justify-between items-center mb-2">
              ...
              {orders.filter(o => ['COLLECTED', 'REJECTED'].includes(o.status)).length > 0 && (
                <button onClick={handleClearHistory} className="text-[10px] font-black text-red-500 uppercase bg-red-50 dark:bg-red-900/20 px-3 py-2 rounded-xl border border-red-100 dark:border-red-900/30">Clear History</button>
              )}
            </div>
            {orders.filter(o => ['COLLECTED', 'REJECTED'].includes(o.status)).map(o => (
              <div key={o.id} className="bg-white dark:bg-gray-900 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 flex justify-between items-center">
                <div>
                  <p className="font-bold text-gray-800 dark:text-white">Order #{o.orderId}</p>
                  <p className={`text-[10px] font-bold ${o.status === 'COLLECTED' ? 'text-green-500' : 'text-red-500'}`}>{o.status}</p>
                </div>
                <button onClick={() => setSelectedHistoryOrder(o)} className="bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 px-4 py-2 rounded-xl font-bold text-xs">Details</button>
              </div>
            ))}
          </div>
        )}
