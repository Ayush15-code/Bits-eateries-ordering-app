  ...
  return (
    <div className="max-w-md mx-auto bg-gray-100 dark:bg-gray-950 min-h-screen pb-20">
      ...
      <div className="p-6">
                ...
                <div key={o.id} className="bg-white dark:bg-gray-900 p-5 rounded-3xl shadow-md border-l-8 border-orange-500">
                  ...
                  <div className="space-y-2">
                    {o.status === "AWAITING_VERIFICATION" && (
                      <button onClick={() => setViewingScreenshot(o.screenshotBase64)} className="w-full py-3 bg-blue-600 text-white rounded-2xl font-black text-[10px] uppercase">View Payment Proof</button>
                    )}
                    <div className="flex gap-2">
                      {o.status === "AWAITING_VERIFICATION" ? (
                        <button onClick={() => fireUpdateDoc(fireDoc(db, "orders", o.id), { status: "CONFIRMED" })} className="flex-1 bg-green-600 text-white py-3 rounded-xl font-black uppercase text-[10px]">Accept Payment</button>
                      ) : (
                        <button onClick={() => fireUpdateDoc(fireDoc(db, "orders", o.id), { status: "COLLECTED", collectedAt: new Date() })} className="flex-1 bg-gray-900 dark:bg-white dark:text-black text-white py-3 rounded-xl font-black uppercase text-[10px]">Mark Collected</button>
                      )}
                    </div>
                  </div>
                </div>
        ...
        {/* --- HISTORY TAB (UPGRADED) --- */}
        {activeTab === 'history' && (
          <div className="space-y-4">
            <p className="text-[9px] font-black text-gray-400 uppercase text-center mb-2 tracking-widest">Automatic Cleanup: Showing Last 2 Hours</p>
            {historyOrders.length === 0 ? (
              <div className="text-center py-20 opacity-40"><History className="mx-auto mb-4" size={48} /><p className="font-bold text-[10px] uppercase">No Recent History</p></div>
            ) : (
              historyOrders.map(o => (
                <div key={o.id} className="bg-white dark:bg-gray-900 p-5 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="text-right">
                      ...
                      <span className={`text-[8px] px-2 py-0.5 rounded font-black uppercase ${o.status === 'REJECTED' ? 'bg-red-100 text-red-600' : 'bg-green-50 text-green-600'}`}>{o.status}</span>
                    </div>
                  </div>

                  <div className="bg-gray-50 dark:bg-gray-800/40 p-3 rounded-2xl mb-3">
                    <p className="text-[10px] font-bold text-gray-600 dark:text-gray-300">
                      {Object.values(getGroupedItems(o.items)).map(i => `${i.quantity}x ${i.name}`).join(', ')}
                    </p>
                  </div>
