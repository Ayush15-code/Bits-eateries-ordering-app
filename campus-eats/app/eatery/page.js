...
export default function EateriesList() {
  ...
  if (snap.exists()) {
    ...
    // Hide bar if COLLECTED or ARCHIVED
...
{activeOrder && isBarVisible && (
  <div className="fixed bottom-6 left-0 right-0 z-50 px-4 flex justify-center">
    <div className="w-full max-w-md bg-orange-600 text-white p-4 rounded-[2rem] shadow-2xl flex items-center justify-between border-2 border-orange-400 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center gap-3">
      ...
      <div className="flex items-center gap-2">
        <Link href={`/status/${activeOrder.id}`}>
          <button className="bg-white text-orange-600 px-5 py-2.5 rounded-2xl font-black text-xs shadow-sm active:scale-95 transition-all">
            Track
          </button>
        </Link>
        ...
        <button
          ...
          className="w-10 h-10 flex items-center justify-center bg-black/10 rounded-full text-white/90 hover:bg-black/20 transition-colors"
        >
          ✕
        </button>
      </div>
