...
export default function Menu() {
      ...
      try {
        ...
        const unsubscribe = onSnapshot(menuRef, (snap) => {
          if (snap.exists()) {
            ...
            // Store hidden categories but DON'T filter the items out
  ...
  const groupedItems = useMemo(() => {
    ...
    filtered.forEach(item => {
      ...
      // REMOVED: if (hiddenCategories.includes(cat)) return;
    ...
    return groups;
  }, [menuItems, searchQuery, hiddenCategories]);
    ...
    <div className="max-w-md mx-auto p-6 bg-white dark:bg-[#050505] min-h-screen pb-32 transition-colors">
      ...
      <div className="space-y-10">
        {Object.keys(groupedItems).map(cat => {
          // CATEGORY AVAILABILITY LOGIC
          const categoryItems = groupedItems[cat];
          const isCategoryHidden = hiddenCategories.includes(cat);
          
          const isCategoryOut = isCategoryHidden || categoryItems.every(group => {
            if (group.isVariantGroup) return group.variants.every(v => v.isAvailable === false);
            ...
            <div key={cat} className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              >
                <div className="flex items-center gap-3">
                  <h2 className={`text-sm font-black uppercase tracking-[0.25em] transition-colors ${isCategoryOut ? 'text-gray-400' : 'text-orange-600'}`}>
                    {cat}
                  </h2>
                  {isCategoryOut && (
                    <span className="text-[8px] font-black bg-red-500/10 text-red-500 px-2 py-0.5 rounded-full uppercase tracking-tighter shadow-sm border border-red-500/20">Sold Out</span>
                  )}
                </div>
              ...
              {openCategories[cat] && (
                <div className="space-y-6">
                  {categoryItems.map(group => {
                    ...
                    // If category is hidden, all items in it become unavailable
                    const isAvailable = !isCategoryHidden && activeItem.isAvailable !== false;

                    return (
                      <div 
                        key={group.id} 
                        className={`group relative flex items-center justify-between p-1 transition-all duration-300 ${!isAvailable ? 'opacity-40 grayscale pointer-events-none' : ''}`}
                      >
                        <div className="flex flex-1 items-start gap-4">
                          <button 
                            onClick={() => isAvailable && toggleFavorite(activeItem.id)} 
                          ...
                          <div className="flex-1">
                            <p className={`font-bold text-gray-800 dark:text-gray-100 text-[15px] tracking-tight leading-tight ${!isAvailable ? 'line-through' : ''}`}>
                              {group.name}
                            </p>
                            ...
                            {isGroup ? (
                              <div className="mt-2 inline-flex items-center gap-2 bg-gray-50 dark:bg-gray-900 px-3 py-1 rounded-xl border border-gray-100 dark:border-gray-800">
                                <select 
                                  ...
                                  className="bg-transparent font-black text-[9px] uppercase outline-none text-orange-600 appearance-none cursor-pointer" 
                                  value={activeItem.id} 
                              ...
                              <p className={`font-black text-sm mt-1 ${!isAvailable ? 'text-gray-400' : 'text-orange-600'}`}>₹{activeItem.price || activeItem.Price}</p>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center ml-4">
                          {isAvailable ? (
                            itemQty > 0 ? (
                              <div className="flex items-center bg-orange-600 text-white rounded-2xl overflow-hidden shadow-xl shadow-orange-500/30 border-2 border-orange-600">
                                <button onClick={() => updateCart(activeItem, -1)} className="p-3 active:scale-90 transition-transform"><Minus size={12} strokeWidth={4} /></button>
                                <span className="w-5 text-center font-black text-xs">{itemQty}</span>
                                <button onClick={() => updateCart(activeItem, 1)} className="p-3 active:scale-90 transition-transform"><Plus size={12} strokeWidth={4} /></button>
                              </div>
                            ) : (
                              <button 
                                onClick={() => updateCart(activeItem, 1)} 
                                className="bg-white dark:bg-[#111] text-orange-600 px-5 py-2.5 rounded-[1.2rem] font-black text-[10px] border-2 border-orange-600/20 hover:border-orange-600 transition-all active:scale-90 uppercase tracking-widest shadow-sm"
                              >
                                Add +
                              </button>
                            )
                          ) : (
                            <div className="bg-gray-100 dark:bg-gray-900 px-3 py-2 rounded-xl">
                              <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest italic">Sold Out</span>
                            </div>
                          )}
                        </div>
                      </div>
      ...
      {cart.length > 0 && (
        <div className="fixed bottom-10 left-0 right-0 px-6 z-50 flex justify-center">
          <div className="w-full max-w-md bg-orange-600 text-white p-5 rounded-[2.8rem] shadow-[0_20px_50px_rgba(249,115,22,0.4)] flex items-center justify-between animate-in slide-in-from-bottom-10 duration-500">
            ...
            <button onClick={() => router.push('/checkout')} className="bg-white text-orange-600 px-10 py-4 rounded-[1.8rem] font-black text-[12px] uppercase tracking-widest active:scale-95 transition-transform">Checkout →</button>
          </div>
        </div>
      )}
    </div>
